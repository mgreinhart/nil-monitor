 ...
 */
const TITLE_RELEVANCE_RE = /\bnil\b|name.image.likeness|\bncaa\b|college\s+athlete|student.athlete|transfer\s+portal|revenue.shar|title\s+ix|college\s+(?:sports?|athletics|football|basketball)|athletic[s]?\s+(?:directors?|departments?|budgets?|deficits?)|\bathletics\b.*(?:private|equity|revenue|invest|budget|deficit)|intercollegiate|\bcompliance\b|\bcollective\b|house\s+v\.?|\bcsc\b|college\s+sports?\s+commission|\buniversit(?:y|ies)\b.*(?:athlet|nil|revenue|private equity|invest|su(?:es?|ed|ing))|\bpower\s+(?:4|5|four|five)\b|conference\s+realignment|\b(?:big\s+ten|big\s+12|big\s+east|pac.12)\b.*(?:deal|rights|equity|revenue|expansion|realign)|\beligibility\b|\bbuyout\b.*(?:college|athlete|transfer|nil|ncaa)|\b(?:lawsuit|injunction|restraining.order)\b.*(?:college|ncaa|athlete|transfer|eligib|athletic)|(?:college|ncaa|athlete|transfer|eligib|athletic).*\b(?:lawsuit|injunction|restraining.order)\b|\bsu(?:es?|ed|ing)\b.*(?:\bncaa\b|college|athlete|quarterback|transfer|eligib)|(?:\bncaa\b|university|college).*\bsu(?:es?|ed|ing)\b/i;
